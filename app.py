...
def route_trips_id(tripid):
    conn = make_connection()
    trips = get_trips_id(tripid, conn)
    return trips.to_json()

def get_trips_id(tripid, conn):
    query = f"""SELECT * FROM trips WHERE id = {tripid}"""
    result = pd.read_sql_query(query, conn)
    return result 
...
@app.route('/trips/average_duration')
    # your code
def route_bike_trips():
    conn = make_connection()
    result = calc_bike_trips(conn)
    return result.to_json()

def calc_bike_trips(conn):
    query=""" pl
    select a.*
    from
        (select  tr.bikeid
...
@app.route('/trips/average_duration/<bike_id>')
    # code here
def route_bikeid_trips(bike_id):
    conn = make_connection()
    result = calc_bikeid_trips(bike_id, conn)
    return result.to_json()

def calc_bikeid_trips(bike_id, conn):
    query=f"""
    select a.*
    from
        (select  tr.bikeid
